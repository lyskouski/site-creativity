...
<article class="el_content">
    ...
    echo $this->partial('Basic/title', array(
        'title' => $this->get('title', $translate->sys('LB_MIND_TECHNIQUE')),
        'title_href' => $this->getUrl('mind/article'),
        'languages' => \Defines\Language::getList(),
        'subtitle' => $translate->sys('LB_CONTENT')
    ));

    echo $this->partial('Basic/Nav/catalog', array(
        'list' => (new \Engine\Validate\Helper\Quote)->convert(\Defines\Catalog::getMind()),
        'url' => 'mind/article/search',
    ...
    if (sizeof($aData['og:title'])):
        ...
        echo $this->partial('Basic/tags_cloud', array(
            ...
            'url' => $this->getUrl('mind/article', false)
        ));
