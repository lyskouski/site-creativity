...
<article class="el_content el_table_pair">
    ...
    echo $this->partial('Basic/title', array(
        'title' => $translate->sys('LB_TASK_MODER_COMMENTS'),
        'title_href' => $this->getUrl('dev/tasks/moder/comments'),
        ...
        ?><section class="el_table indent">
            <header>
                <small class="right">
                    ...
                    ?>
                    <br />
                    <a class="right button bg_attention cr_pointer ui" data-class="Request/Pjax" data-actions="init" data-href="<?php echo $this->getUrl('dev/tasks/moder/comments', \Defines\Extension::JSON) ?>" data-data="{'action':'decline', 'id':<?php echo $o->getId() ?>}"><?php echo $translate->sys('LB_BUTTON_DECLINE') ?></a>
                    <a class="right button bg_normal cr_pointer ui" data-class="Request/Pjax" data-actions="init" data-href="<?php echo $this->getUrl('dev/tasks/moder/comments', \Defines\Extension::JSON) ?>" data-data="{'action':'approve', 'id':<?php echo $o->getId() ?>}" ><?php echo $translate->sys('LB_BUTTON_APPROVE') ?></a>
                </small>
                <?php
                if (\System\Registry::user()->checkAccess($this->get('module_url'), 'modify')):
                    ?><a class="right bg_attention cr_pointer ui" data-class="Request/Pjax" data-actions="init" data-href="<?php echo $this->getUrl($o->getPattern(), \Defines\Extension::JSON) ?>" data-data="{'action':'modify', 'id':<?php echo $o->getId() ?>}" title="<?php echo $translate->sys('LB_BUTTON_MODIFY') ?>">
